...
const ORG_PATH = `${pages['organization']}?dao=${testDaos.ERC20.value}`;

const test = new BaseSeleniumTest('organization', 'organization/content-loads');
BaseSeleniumTest.run(async (test) => {
  await test.start();
  // Load the organization page
  await test.driver!.get(getBaseUrl() + ORG_PATH);
